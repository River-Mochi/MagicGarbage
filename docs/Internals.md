...
| Total Magic – cleanup pass         | Wipes garbage counters, requests, and warning flags on all garbage producers while enabled.       | `MagicGarbageSystem` Burst `MagicJob` iterates `GarbageProducer` chunks and clears state **only when** `setting.TotalMagic` is true. |
...
| Mod bootstrap                      | Registers settings, locales, and hooks main systems into the GameSimulation phase.                | `Mod.OnLoad` adds locale sources, loads settings, registers Options UI, and schedules `MagicGarbageSystem`, `GarbageTruckCapacitySystem`, `GarbageFacilityCapacitySystem`. |
